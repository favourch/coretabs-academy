          ...
          <v-layout fluid row justify-center align-center wrap>
            ...
            <v-flex id="form" xs10 sm10 md5>
              <v-form ref="form" lazy-validation>
                <v-text-field dir="auto" :hint="i18n.username_hint" :label="form.fullname_label" v-model.trim="fullname" :class="[alert.success ? 'disabled' : '']" :rules="fnRules" :disabled="alert.success" @keyup.enter="submit" autocomplete="on" name="name" required autofocus></v-text-field>
                <v-text-field dir="auto" :label="form.email_label" v-model.trim="email" :class="[alert.success ? 'disabled' : '']" :rules="emRules" :disabled="alert.success" @keyup.enter="submit" autocomplete="on" name="email" required></v-text-field>
                <v-text-field dir="auto" :label="form.username_label" v-model.trim="username" :class="[alert.success ? 'disabled' : '']" :rules="unRules" :disabled="alert.success" @keyup.enter="submit" required></v-text-field>
