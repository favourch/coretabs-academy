...
const AuthAPI = {
      ...
      axios.patch('/api/v1/auth/user/', formData, {
         ...
         if (root.$store.getters.user('email') === root.email) {
            ...
            if (root.$store.getters.user('avatar_url') !== root.avatar_url) {
               root.$store.dispatch('user', {
                  prop: 'avatar_url',
                  data: '/media/avatars/' + root.username + '/resized/80/' + root.validImage.imageData.name
               })
            }
   ...
   selectTrack(root) {
      ...
      return axios.patch('/api/v1/auth/user/', {
         ...
         root.$store.dispatch('profile', {
            prop: 'track',
            data: root.track_selected
         })
         this.updateUser(root.$store)
         return true
      })
   ...
   storeUser(store, data = null) {
      if (data !== null) {
         ...
         return store.dispatch('user', {
               prop: null,
               data: JSON.parse(Vue.prototype.$encryption.b64DecodeUnicode(window.localStorage.getItem('user')))
            })
            .then((response) => {
               return response
            })
   ...
   updateUser(store) {
      window.localStorage.setItem('user', Vue.prototype.$encryption.b64DecodeUnicode(JSON.stringify(store.getters.user(null))))
   },
   removeUser(store) {
